...
const String yt="2.233.128.13";
String url = "http://"+yt+"/state";
String payload;
...
void fetchJsonData() {
  ...
  if (httpResponseCode > 0) {
    ...
    Serial.println("-----line 32 payload-------");
...
//sendButtonState();
  ...
  Serial.println("_Line 105_-----in loop-----buttons ===============");
  ...
  for (JsonVariant button : buttons) {
    ...
    Serial.print(" line 113 buttonLabel: ");
    Serial.print(label);
    Serial.print(" is @ pin: ");
    Serial.print(buttonPins[i]);
    Serial.print(" in loop and the state is: ");
    Serial.println(state);
//sendButtonStateP(int buttonId, const char* state, const char* buttonLabel, int buttonPin) ;
  sendButtonStateP(i, state, label, buttonPins[i]) ;
...
StaticJsonDocument<256> jsonPayload;
  ...
  String urpl="2.233.128.13";
  // Send the HTTP POST request
  HTTPClient http;
  http.begin("http://"+urpl+"/Pstate");
...
void sendButtonStateP(int buttonId, const char* state, const char* buttonLabel, int buttonPin) {
  HTTPClient http;
  String path ="http://"+yt+"/Pstate";
  
    Serial.print(" line 175 path : ");
    Serial.println(path);
  http.begin("http://2.233.128.13/Pstate");
  http.addHeader("Content-Type", "application/json");
 ...
 doc["buttonPin"] = buttonPin;
   ...
   //doc["buttonPin"] = 5;
    ...
    Serial.print("buttonId   line 187 : ");
    Serial.println(buttonId);

    
    Serial.print("line 191 buttonLabel use &: ");
    Serial.println(buttonLabel);

    
    Serial.print("buttonPin line 194 : ");
    Serial.println(buttonPin);
  // Serialize the JSON document to a string
  String payload;
  serializeJson(doc, payload);
Serial.print("Line 190 payload--   ");
Serial.println(payload);
